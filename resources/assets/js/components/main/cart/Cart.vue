			...
			<div class="table-responsive">
				...
				<table class="shop_table beta-shopping-cart-table" cellspacing="0">
					...
					<tbody>
						...
						<tr class="cart_item" v-for="(item,index) in cart" :key="index">
							<td class="product-name">
								<div class="media">
									<img class="pull-left" :src="`/image/product/${ item.image }`" alt="">
									
								</div>
							</td>

							<td class="product-price">
								<span class="amount">{{ item.price| currency(' Đ', 0, { symbolOnLeft: false }) }}</span>
							</td>

						
							<td class="product-quantity">
								<!-- <select name="product_qty" id="product-qty" @change="()=>onChange(item.id,index,qty)">
									
									<option v-for="opt in option" v-bind:value="opt.value" :key="opt.id" >
										{{ opt.text}}
									</option>
								
								</select> -->
								 <button class="ch-le" @click="onChange(-1,item.qty,index)">  -  </button><span> {{ item.qty }}</span><button class="ch-ri" @click="onChange(1,item.qty,index)">  +  </button>
							</td>

							<td class="product-subtotal">
								<span class="amount">{{ item.price*item.qty | currency(' Đ', 0, { symbolOnLeft: false }) }}</span>
							</td>

							<td class="product-remove">
								<a href="#" class="remove" title="Remove this item" @click="deleteCart(item.id,index)"><i class="fa fa-trash-o"></i></a>
							</td>
						</tr>
			...
			<div class="cart-collaterals">
				...
				<form class="shipping_calculator pull-left" action="#" method="post">
					<h2><a href="#" class="shipping-calculator-button">Calculate Shipping <span>↓</span></a></h2>
						<router-link tag="button" type="submit" class="beta-btn primary" to="/dathang" name="apply_coupon">DAT HANG <i class="fa fa-chevron-right"></i></router-link>
				...
				<div class="cart-totals pull-right">
					...
					<div class="cart-totals-row"><span>Cart Subtotal:</span> <span>{{ this.total_cart()| currency(' Đ', 0, { symbolOnLeft: false }) }}</span></div>
...
<script>
   ...
   export default {
	   ...
	   methods:{
		   ...
		   onChange(st,qty1,index){
			  var qty=qty1+st;
			   var arr={ index,qty };
			   this.$store.dispatch('changqty',arr);
			  
		   },
		   deleteCart(id,index){
			   var arr= { id,index};
			     this.$store.dispatch('deleteCart',arr);
		   }
